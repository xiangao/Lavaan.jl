...
In `Lavaan.jl`, variables can be declared as ordinal. The estimator then uses
diagonally weighted least squares (DWLS), which is the usual starting point for
Likert-scale or binary indicators.
...
The point is to avoid treating ordinal indicators as continuous when that is not
the model one wants to estimate.
